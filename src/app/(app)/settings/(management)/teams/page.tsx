      ...
      {!league ? (
        ...
        <>
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-2 mb-3 -mx-4 px-4 scrollbar-none">
            {competitions.map(c => (
              <button key={c.id} onClick={() => handleCompChange(c.id)} className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${activeComp === c.id ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-secondary)]'}`}>
                {c.short_name}
              </button>
            ))}
          </div>
              ...
              <div key={country}>
                ...
                <div className="space-y-1.5">
                  {teams.map(team => {
                    ...
                    return (
                      <button key={team.id} onClick={() => toggleTeam(team.id)} className={`w-full text-left rounded-xl border p-3 transition-colors ${isSelected ? 'border-[var(--accent)]/60 bg-[var(--accent)]/10' : 'border-[var(--border)] bg-[var(--bg-card)] hover:bg-[var(--bg-card-hover)]'}`}>
