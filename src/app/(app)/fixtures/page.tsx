...
type FixtureWithRelations = Fixture & { competition: Competition; home_team: Team; away_team: Team }
...
export default function FixturesPage() {
  ...
  async function loadData() {
    ...
    const [{ data: comps }, { data: fix }, { data: teamData }] = await Promise.all([
      ...
      supabase.from('fixtures').select(`*, competition:competitions(*), home_team:teams!fixtures_home_team_id_fkey(*), away_team:teams!fixtures_away_team_id_fkey(*)`).eq('league_id', lg.id).order('kickoff_time', { ascending: true }),
    ...
    for (const row of (teamData ?? []) as any[]) {
      if (row.teams && !seen.has(row.teams.id)) { seen.add(row.teams.id); uniqueTeams.push(row.teams) }
    }
  ...
  const filteredFixtures = fixtures.filter(f => {
    const statusMatch = activeTab === 'upcoming' ? (f.status === 'scheduled' || f.status === 'live') : f.status === 'completed'
    return statusMatch && (activeCompetition === 'all' || f.competition_id === activeCompetition)
  })

  if (loading) return <AppShell title="Fixtures"><PageLoader /></AppShell>

  return (
    <AppShell title="Fixtures" action={<Button size="sm" onClick={() => setAdding(true)}>+ Add</Button>}>
      <TabBar tabs={[{ key: 'upcoming', label: 'Upcoming' }, { key: 'results', label: 'Results' }]} active={activeTab} onChange={v => setActiveTab(v as any)} className="mb-3" />

      {competitions.length > 0 && (
        <div className="flex gap-1.5 overflow-x-auto pb-2 mb-3 -mx-4 px-4 scrollbar-none">
          <button onClick={() => setActiveCompetition('all')} className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium border transition-colors ${activeCompetition === 'all' ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-secondary)] hover:border-[var(--accent)]'}`}>All</button>
          {competitions.map(c => (
            <button key={c.id} onClick={() => setActiveCompetition(c.id)} className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium border transition-colors ${activeCompetition === c.id ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--border)] text-[var(--text-secondary)] hover:border-[var(--accent)]'}`}>{c.short_name}</button>
          ))}
        </div>
      )}

      {filteredFixtures.length === 0 ? (
        <EmptyState icon={activeTab === 'upcoming' ? '📅' : '📊'} title={activeTab === 'upcoming' ? 'No upcoming fixtures' : 'No results yet'} description={activeTab === 'upcoming' ? 'Check back after the season starts.' : 'Results will appear here once matches are completed.'} />
      ) : (
        <div className="space-y-2">
          {filteredFixtures.map(f => <FixtureCard key={f.id} fixture={f} onUpdate={loadData} />)}
        </div>
      )}

      {adding && league && <AddFixtureModal leagueId={league.id} competitions={competitions} teams={teams} onClose={() => setAdding(false)} onSaved={() => { setAdding(false); loadData() }} />}
    </AppShell>
  )
}

function FixtureCard({ fixture, onUpdate }: { fixture: FixtureWithRelations; onUpdate: () => void }) {
  ...
  return (
    <Card className="!p-3">
      <div className="flex items-center gap-1.5 text-[10px] text-[var(--text-muted)] mb-1.5">
        <Badge variant={fixture.competition?.competition_type === 'european' ? 'purple' : 'default'} className="text-[9px]">{fixture.competition?.short_name}</Badge>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="shrink-0 text-center min-w-[56px]">
          {fixture.status === 'completed' ? (
            <span className="font-bold text-sm text-[var(--text-primary)]">{fixture.home_score} – {fixture.away_score}</span>
          ) : fixture.status === 'live' ? (
      ...
      {fixture.status !== 'completed' && (
        <div className="mt-2">
          {editing ? (
            ...
            <Button size="sm" variant="ghost" onClick={() => setEditing(true)} className="text-[10px]">Enter result</Button>
          )}
        </div>
      )}
    </Card>
...
async function updateTeamScores(leagueId: string, fixture: any) {
  const supabase = createClient()
  const h = fixture.home_score, a = fixture.away_score
  async function upsertScore(teamId: string, isHome: boolean) {
    const { data: existing } = await supabase.from('team_scores').select('*').eq('league_id', leagueId).eq('team_id', teamId).maybeSingle()
    const my = isHome ? h : a, opp = isHome ? a : h
    const win = my > opp ? 1 : 0, draw = my === opp ? 1 : 0, loss = my < opp ? 1 : 0
    const pts = win ? 3 : draw ? 1 : 0
    if (existing) {
      await supabase.from('team_scores').update({ total_points: (existing.total_points ?? 0) + pts, wins: (existing.wins ?? 0) + win, draws: (existing.draws ?? 0) + draw, losses: (existing.losses ?? 0) + loss, goals_for: (existing.goals_for ?? 0) + my, goals_against: (existing.goals_against ?? 0) + opp, matches_played: (existing.matches_played ?? 0) + 1, last_calculated_at: new Date().toISOString() }).eq('id', existing.id)
    } else {
      await supabase.from('team_scores').insert({ league_id: leagueId, team_id: teamId, total_points: pts, wins: win, draws: draw, losses: loss, goals_for: my, goals_against: opp, matches_played: 1 })
    }
  }
  await Promise.all([upsertScore(fixture.home_team_id, true), upsertScore(fixture.away_team_id, false)])
...
async function recalcPlayerScores(leagueId: string) {
  ...
  for (const player of (players ?? [])) {
    ...
    for (const a of playerTeams) {
      ...
      if (ts) { total += ts.total_points ?? 0; wins += ts.wins ?? 0; draws += ts.draws ?? 0; losses += ts.losses ?? 0; played += ts.matches_played ?? 0 }
    }
    const { data: existing } = await supabase.from('player_scores').select('id').eq('league_id', leagueId).eq('player_id', player.id).maybeSingle()
    if (existing) {
      await supabase.from('player_scores').update({ total_points: total, wins, draws, losses, matches_played: played, last_calculated_at: new Date().toISOString() }).eq('id', existing.id)
    } else {
      await supabase.from('player_scores').insert({ league_id: leagueId, player_id: player.id, total_points: total, wins, draws, losses, matches_played: played })
    }
  }
}

function AddFixtureModal({ leagueId, competitions, teams, onClose, onSaved }: { leagueId: string; competitions: Competition[]; teams: Team[]; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setSaving(true)
    const supabase = createClient()
    await supabase.from('fixtures').insert({ league_id: leagueId, competition_id: form.competition_id, home_team_id: form.home_team_id, away_team_id: form.away_team_id, kickoff_time: form.kickoff_time || null, round: form.round || null, matchday: form.matchday ? parseInt(form.matchday) : null, status: 'scheduled' })
