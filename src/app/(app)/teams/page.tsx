...
export default function TeamsPage() {
  ...
  async function load() {
    ...
    for (const row of (teamCompData ?? []) as any[]) {
      ...
      if (!team || teamMap.has(team.id)) continue
      teamMap.set(team.id, {
        ...team,
  ...
  return (
    <AppShell title="Teams">
      ...
      {!hasTeams ? (
        <EmptyState icon="⚽" title="No teams selected" description="Choose the Premier League team pool and run the draft to see teams here." />
      ) : (
