...
import type { League, Player, Team, DraftRun } from '@/lib/supabase/types'
...
export default function DraftPage() {
  ...
  async function loadData() {
    ...
    const [
      ...
      supabase.from('team_competitions').select('team_id, competition_id, teams(*), competitions(competition_type)').eq('league_id', lg.id),
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (draftTeams.length < players.length * 5) throw new Error(`Need at least ${players.length * 5} teams (have ${draftTeams.length}). Add more teams to competitions.`)
      const result = runDraft(players.map(p => ({ id: p.id, name: p.name, color: p.color })), draftTeams, europeanIds)
  ...
  const currentAlloc = players.map(p => {
    ...
    return { player: p, teams: teams.filter(Boolean), euCount: teams.filter((t: any) => t && europeanIds.has(t.id)).length }
  })
    ...
    <AppShell title="Draft Room">
      ...
      <Card className="mb-4">
        ...
        <div className="space-y-1.5">
          <Req ok={players.length === 12} label={`Players: ${players.length}/12`} />
          <Req ok={draftTeams.length >= players.length * 5} label={`Teams: ${draftTeams.length} (need ${players.length * 5})`} />
          <Req ok={europeanIds.size > 0} label={`European teams: ${europeanIds.size}`} />
                ...
                <div className="flex flex-wrap gap-1.5">
                  {entry.teams.map((team: any) => team && (
                    <div key={team.id} className="flex items-center gap-1">
                      ...
                      {europeanIds.has(team.id) && <span className="text-[8px] text-purple-400">★</span>}
                    </div>
                  ))}
                </div>
              </Card>
